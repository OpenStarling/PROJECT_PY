...
#print("oil retention time(minute):")
#oil_retention_time = float(input())
oil_retention_time = 10

#print("water retention time(minute):")
#water_retention_time = float(input())
water_retention_time = 10

h_oil_max = (1.28 * 10**(-3))*((oil_retention_time * delta_SG * (dm_water**2))/(miuoil))
print("h_oil_max=", h_oil_max)

ratios_of_A = 0.5*(Qwater * water_retention_time)/(oil_retention_time * Qoil + water_retention_time * Qwater)
print("ratios_of_A=", ratios_of_A)


BetaList = [0.5, 0.4, 0.35, 0.29, 0.24, 0.2, 0.16, 0.12, 0.08, 0.04, 0.001]
AwaList = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]

min = 100
k = 0

for i in range(11):
    if abs(AwaList[i]-ratios_of_A) < min:
        k = i
        min = abs(AwaList[i]-ratios_of_A)
...
d_sq_l_eff = 1.42 * (Qwater * water_retention_time + Qoil * oil_retention_time)
print("D_sq_Leff=", d_sq_l_eff)
dList = []
LeffList = []
LssList = []
SRList = []
...
for i in range(150):
    dList.append(dmax)
    Leff = d_sq_l_eff/(dmax**2)
    LeffList.append(Leff)
    Lss = (4 * Leff)/3
    LssList.append(Lss)
    SR = 12 * Lss / dmax
    SRList.append(SR)
...
for i in range(150):
    print(SRList[i], end = " ")
    if 3 < SRList[i] < 4 and bolean == 0:
        x = i
        bolean = 1
        
print("x = ", x)

print("d = ", dList[x], "inches")
print("Leff = ", LeffList[x], "ft")
print("Lss = ", LssList[x], "ft")
...
length_ft = LssList[x]  # Длина цилиндра в футах
diameter_inch = dList[x]  # Диаметр цилиндра в дюймах
...
num_length_points = 50  # Точек вдоль длины

# Сетка углов и длины
theta = np.linspace(0, 2 * np.pi, num_sides)
x = np.linspace(0, length_ft, num_length_points)
theta_grid, x_grid = np.meshgrid(theta, x)

# Координаты поверхности цилиндра
y_grid = radius_ft * np.cos(theta_grid)  # Радиус по оси Y
z_grid = radius_ft * np.sin(theta_grid)  # Радиус по оси Z
...
# Настройка пропорций для правильного отображения
ax.set_box_aspect([length_ft, 2 * radius_ft, 2 * radius_ft])  # Пропорции для горизонтального цилиндра
...
vertices = []
for x_val in x:  # Проходим по длине цилиндра
    for angle in theta:  # Проходим по окружности
        y = radius_ft * np.cos(angle)
        z = radius_ft * np.sin(angle)
        vertices.append([x_val, y, z])

# 2. Создаём треугольники для поверхности цилиндра
faces = []

# Формируем грани цилиндра
for i in range(num_length_points - 1):
    for j in range(num_sides - 1):
        # Индексы текущей точки и соседних точек
        v1 = i * num_sides + j
        v2 = v1 + 1
        v3 = v1 + num_sides
        v4 = v3 + 1
        # Два треугольника между четырьмя точками
...
# Сохраняем STL-файл
cylinder_mesh.save('horizontal_cylinder.stl')
print("STL файл 'horizontal_cylinder.stl' успешно создан!")
